...
class QRectCreator {
    ...
    buildShape() {
        ...
        switch (this.shapeType) {
            case "line":
                return new Line(rect.p1, rect.p2, qview.lineStyle)
            case "rect":
                return new Rect(r, qview.lineStyle)
            case "ellipse":
                let rx = r.width / 2
                let ry = r.height / 2
                return new Ellipse(r.x + rx, r.y + ry, rx, ry, qview.lineStyle)
            case "circle":
                let rc = Math.sqrt(r.width * r.width + r.height * r.height)
                return new Ellipse(rect.p1.x, rect.p1.y, rc, rc, qview.lineStyle)
            default:
